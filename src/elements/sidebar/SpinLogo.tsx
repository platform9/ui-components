...
const useLogoStyles = makeStyles<Theme, { active?: boolean }>((theme) => ({
  logoContainer: {
    display: 'grid',
  },
  spinContainer: {
...
const CustomLogo = ({ logoSrc, logoUrl }) => {
  ...
  return <img className={classes.customLogo} src={imageSource || logoUrl} />
}

export default function SpinLogo({ active = false }) {
  const classes = useLogoStyles({ active })
  return <div className={classes.spinContainer}>{logoDefault}</div>
