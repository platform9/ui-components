...
function Dropdown(props) {
    ...
    const initialSelectedItem = (0, react_1.useMemo)(() => {
        if (!(0, ramda_1.isNil)(initialValue) && !noBlankValue) {
    ...
    const selectedItem = (0, react_1.useMemo)(() => {
        if (showAll && ((0, ramda_1.isNil)(value) || value === allKey)) {
            return showAllItem;
        }
        if (showNone && ((0, ramda_1.isNil)(value) || value === noneKey)) {
            return showNoneItem;
        }
        if (!(0, ramda_1.isNil)(value) && !Array.isArray(value)) {
    ...
    const getFilteredItems = (0, react_1.useCallback)((inputValue) => {
        const menuItems = [];
        if (showAll) {
    ...
    return (react_1.default.createElement(downshift_1.default, { initialSelectedItem: initialSelectedItem, selectedItem: onChange ? selectedItem : null, onChange: onChange ? handleChange : null, itemToString: itemToString }, ({ getInputProps, getToggleButtonProps, getMenuProps, selectItem, isOpen, clearSelection, inputValue, toggleMenu, getLabelProps, }) => (react_1.default.createElement("div", { className: (0, clsx_1.default)(classes.dropdownWrapper, 'dropdown-container', className) },
        label && (react_1.default.createElement(Text_1.default, Object.assign({ "data-testid": (0, test_helpers_1.default)(label, 'dropdown', 'label') }, getLabelProps(), { onClick: () => {
                var _a;
                toggleMenu();
        ...
        react_1.default.createElement("div", { className: (0, clsx_1.default)(classes.inputFrame, 'inputFrame', {
            ...
            react_1.default.createElement(DropdownMenu_1.default, Object.assign({ compact: compact, width: width, isOpen: isOpen }, getMenuProps()), isOpen
                ? getFilteredItems(inputValue).map((item) => {
                    var _a, _b;
                    return (react_1.default.createElement(DropdownItem_1.default, { "data-testid": (0, test_helpers_1.default)('dropdown', 'selection', 'value'), key: (_a = item.key) !== null && _a !== void 0 ? _a : String(item.value), onClick: item.disabled ? null : () => selectItem(item), disabled: item.disabled }, (_b = item.content) !== null && _b !== void 0 ? _b : react_1.default.createElement("span", null, itemToString(item))));
                })
                : null)),
        !!error && (react_1.default.createElement(Text_1.default, { "data-testid": (0, test_helpers_1.default)('dropdown', 'error'), variant: "body2", className: (0, clsx_1.default)(classes.error, 'error') }, error))))));
}
