...
const CardTable = ({ data, searchTarget, filters, filterValues, showSortOption = false, sortOptions = [], onSortChange, sortBy = 'asc', sortTarget, loading, loadingMessage, handleRefresh, children, emptyItemsMessage, }) => {
    ...
    const filter = react_1.useCallback((data) => {
        if (!filters)
            return data;
        ...
        filterValues.map(({ value, target, customFilterFn }) => {
            if (value && value !== constants_1.allKey) {
                filteredData = customFilterFn
                    ...
                    : filteredData.filter((item) => fp_1.pathStrOr('', target, item) === value);
            }
        });
        return filteredData;
    }, [filterValues]);
    const filterBySearch = react_1.useCallback((data) => {
        return data.filter((item) => ramda_1.pathOr('', searchTarget.split('.'), item).match(new RegExp(searchTerm, 'i')) !== null);
    }, [searchTerm, searchTarget]);
    const sort = react_1.useCallback((data) => {
        if (!showSortOption || !sortTarget) {
            return data;
        }
        const sortedData = data.sort((a, b) => {
            const aValue = ramda_1.pathOr('', sortTarget.split('.'), a);
            const bValue = ramda_1.pathOr('', sortTarget.split('.'), b);
            return aValue.toLowerCase() > bValue.toLowerCase() ? 1 : -1;
        });
        return sortBy === 'desc' ? sortedData.reverse() : sortedData;
    }, [showSortOption, sortTarget, sortBy]);
    const filteredData = ramda_1.compose(filter, filterBySearch, sort)(data);
    const hasData = react_1.useMemo(() => (filteredData === null || filteredData === void 0 ? void 0 : filteredData.length) > 0, [filteredData]);
